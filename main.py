...
games1 = {
    1: {'name': 'Chain Cube 2048'},
    2: {'name': 'Train Miner'},
    3: {'name': 'Merge Away'},
    4: {'name': 'Twerk Race 3D'},
    5: {'name': 'Polysphere'},
    6: {'name': 'Mow and Trim'},
    7: {'name': 'Tile Trio'},
    8: {'name': 'Zoopolis'},
...
output = "Select a game:\n" + "\n".join([f"{key}: {value['name']}" for key, value in games1.items()])
print(output)

ac = int(input("masukan pilihan game: "))

async def countdown(seconds):
    while seconds:
        mins, secs = divmod(seconds, 60)
        time_format = f"{mins:02d}:{secs:02d}"
        sys.stdout.write(f"\rWaiting for next attempt in: {time_format}")
        sys.stdout.flush()
        await asyncio.sleep(1)
        seconds -= 1
    sys.stdout.write("\rCountdown complete! Starting next attempt...\n")
...
if __name__ == "__main__":
    async def main_loop():
        ...
        while True:
            ...
            logger.info("t.me/rumahberbagiduafa")
            await countdown(60)  # Countdown for 10 minutes (600 seconds)
